...
// O CSS foi movido para dentro do componente para facilitar.
// A linha 'import ./Visor.css' não é mais necessária.

function Visor() {
  // Estado para armazenar a lista de todas as senhas já chamadas
  ...
  const carregarSenhas = () => {
    try {
      ...
      // Em caso de erro, define como um array vazio para evitar que a aplicação quebre
      setSenhasChamadas([]);
    }
  };

  // useEffect para carregar os dados iniciais e ouvir por mudanças
  useEffect(() => {
    // 1. Carrega os dados quando o componente é montado pela primeira vez
    carregarSenhas();

    // 2. Adiciona um event listener para o evento 'storage'
    // Este evento é disparado quando o localStorage é alterado em outra aba
    ...
    // 3. Função de limpeza: remove o event listener quando o componente é desmontado
    return () => {
      window.removeEventListener('storage', handleStorageChange);
    };
  }, []); // O array vazio [] garante que este useEffect execute apenas uma vez (montagem e desmontagem)

  // Determina qual senha é a principal e quais são as últimas
  ...
  return (
    <>
      {/* O CSS está aqui dentro para manter o componente auto-contido */}
      <style>
        {`
          * {
